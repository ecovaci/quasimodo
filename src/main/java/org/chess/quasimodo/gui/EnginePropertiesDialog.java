...
public class EnginePropertiesDialog extends  AbstractDialogForm<EngineModel> {//TODO - refactoring
...
/*	*//**
	...
	public static void main(String[] args) {
		try {
			EnginePropertiesDialog dialog = new EnginePropertiesDialog();
	...
	private void addOptionValue (Option option) {
		...
		if (option.isTypeSpin()) {
			...
			((JSpinner)component).setValue(
					option.value != null ? Integer.parseInt(option.value):Integer.parseInt(option.defaultValue));
		} else if (option.isTypeCheck()) {
			component = new JCheckBox();
			((JCheckBox)component).setSelected(
					option.value != null ? Boolean.parseBoolean(option.value):Boolean.parseBoolean(option.defaultValue));
	...
	@Override
	public void commit() {//FIXME - wrong code in commit (add components)
		...
		rightPanel.setPreferredSize(new Dimension(rightPanel.getWidth(), 
				100 + 24 * (idOptions.getOptions().size() + 2) + 2));
		...
		btnChooseBook.addActionListener(new ActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent e) {
				JFileChooser fileChooser = new JFileChooser();//TODO - point to openingBook dir
